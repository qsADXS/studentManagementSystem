...
public class TeacherController {
    ...
    public Map<String, Object> grade(@PathVariable String crouseId, @PathVariable String studentId, @RequestBody Map<String, Object> gradeData, HttpServletRequest request) {
        Claims claims = JwtUtils.getClaims(request);
        int level = (int)claims.get("level");
        if(level < 2){
            throw new DefinitionException(ErrorEnum.NO_PERMISSION);
